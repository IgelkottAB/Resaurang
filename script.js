...
function renderTables(){const el=document.getElementById("tablesGrid");el.innerHTML=state.tables.map(t=>`<div class="table-card ${t.status}" onclick="openOrder(${t.id})"><div class="table-top"><b>Bord ${t.id}</b><span>${t.status==="free"?"Ledigt":t.status==="pay"?"Betalning":t.order?.status==="ready"?"Klar":"Upptaget"}</span></div><div class="table-meta">${t.order?`${t.order.items.reduce((s,x)=>s+x.qty,0)} artiklar`:"Inget aktivt sällskap"}</div><div class="table-total">${t.order?kr(total(t)):"—"}</div></div>`).join("")}
...
function importCustomerOrder(o){
  ...
  if(t.order){
    toast(`Ny kundbeställning till bord ${o.table} kunde inte läggas: bordet har redan en order.`);
    return;
  }
