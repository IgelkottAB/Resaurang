...
function getCustomers(){
  try{
    const raw=JSON.parse(localStorage.getItem("igelkott_demo_cards")||"null");
    if(raw && raw.cards) return raw.cards;
...
async function saveMenuItem(){const id=+document.getElementById("editItemId").value,name=document.getElementById("itemName").value.trim(),cat=document.getElementById("itemCategory").value.trim(),price=+document.getElementById("itemPrice").value,emoji=document.getElementById("itemEmoji").value.trim()||"🍽️";if(!name||!cat||price<0)return toast("Fyll i namn, kategori och giltigt pris.");if(id)Object.assign(state.menu.find(x=>x.id===id),{name,category:cat,price,emoji});else state.menu.push({id:Date.now(),name,category:cat,price,emoji});save();await publishMenu();document.getElementById("itemModal").classList.add("hidden");renderAll();toast("Meny uppdaterad för kundsidan.")}
async function deleteMenuItem(id){if(confirm("Ta bort denna rätt?")){state.menu=state.menu.filter(x=>x.id!==id);save();await publishMenu();renderAll();toast("Meny uppdaterad för kundsidan.")}}
...
async function publishMenu(){
  const cfg=ntfyConfig();
  if(!cfg.menuTopic) return false;
  return publishNtfyToTopic(cfg.menuTopic,"menu_update",{menu:state.menu});
}

async function publishNtfyToTopic(topic,kind,payload){
  const cfg=ntfyConfig();
  if(!topic) return false;
  const message=JSON.stringify({kind,payload,ts:Date.now()});
  try{
    const r=await fetch(cfg.base+"/"+encodeURIComponent(topic),{method:"POST",headers:{"Content-Type":"text/plain","Title":"Igelkotts restaurang – meny"},body:message});
    return r.ok;
  }catch(e){console.warn("Kunde inte uppdatera kundmenyn:",e);return false}
}
